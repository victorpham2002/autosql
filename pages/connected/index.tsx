...
const ChattingLog = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const sliceChatLog = updatedChatLog
      .slice(-MAX_LITMIT_MESSAGES)
      .filter((message) => filterRoles.includes(message.role));
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a professional database developer. You can create SQL query with high accuracy.",
          },
          {
            role: "user",
            content:
              "I am using PostgreSQL. I have a database with the following description in CSV format. Each row is a table. The first value of every rows is the table name, and the next values are all columns of that table.",
          },
          { role: "user", content: JSON.stringify(tableData) },
          {
            role: "user",
            content:
              "I will describe my data requirements, you will provide the SQL query for me to get the data I want.",
          },
          {
            role: "user",
            content:
              "Note that you must not say anything else. You must provide me only the SQL query",
          },
          {
            role: "user",
            content:
              'Example format for the return message: "SELECT * FROM users ORDER BY user_id ASC"',
          },
    ...
    setChatLog((prevChatLog) => [
      ...prevChatLog,
      { role: "assistant", content: res.firstResponse },
    ]);
    setDisplayChatLog((prevChatLog) => [
      ...prevChatLog,
      { role: "assistant", content: res.firstResponse },
  ...
  return (
    <div className="container mx-auto w-full mb-20 mt-20">
        ...
        {displayChatLog.map((message, index) => (
          ...
          >
            <div
              className={`${
                message.role === "user"
                  ? "bg-button text-white lg:mr-10"
                  : "lg:ml-10 border-2 border-slate-500"
              } relative rounded-xl p-3 max-w-[280px] md:max-w-md `}
            >
          ...
          >
            <div className="flex flex-col justify-between h-full w-full">
              <div className=" overflow-y-auto overflow-x-auto">
                ...
                {isLoading ? (
                  <div className="max-w-full text-center text-3xl m-8">
                    Loading...
                  </div>
                ) : (
      ...
      <form
        className="flex justify-center items-center bg-white shadow border-t-2 rounded p-3 fixed bottom-0 left-0 w-full"
